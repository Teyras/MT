...
programming assignments, using ReCodEx as a model.
...
This mechanism is useful for efficient updates and extension of images.
...
Containers are often configured using environment variables passed from the host 
when they are created. It is also possible to bind network ports of a container 
to host ports and to mount parts of the host file system into a container.
...
making changes to it manually and then saving the writable layer of the 
...
Docker. The daemon can upload images to the registry with the `push` command
and download them with the `pull` command. Both of these commands utilize 
layers, which makes transferring the images efficient.
...
with `isolate`. In the default setup, it misses measurements and limiting of 
resource usage (time, memory, disk usage, ...). With further configuration, a 
memory limit can be set. However, adding time and disk usage limits and 
gathering statistics after the evaluated solution finishes would require 
implementing a supervisor program instead of just launching Docker containers 
...
However, there are ways in which it could complement it. Currently, runtime 
environments lack a precise definition and rely on the dilligence of 
administrators to function correctly.
...
required by a particular set of runtime environments. This approach is similar 
to supplying custom runners in GitLab CI. The alternative is packaging each 
runtime environment in a separate image, keep the worker separated and implement 
launching images in the worker, which is more similar to how CircleCI and GitLab 
CI work with containers.

The first approach allows things such as running specialized, heavily modified 
versions of the worker itself, which might be needed by some future exercises 
(tasks such as running a program on a remotely controller cluster of servers). 
...
be smaller, easier to review and faster to build. Also, a single worker could 
...
### Launching Containers with `isolate`
...
kernel, supporting the other storage backends is not much of a concern. The 
second, less complicated way of accessing the image contents is unpacking the 
images only when they are downloaded. Then, when the image is needed, we could 
use `overlay2` to link the image contents to the working directory of `isolate`. 
A drawback of this approach is that it is not as efficient with disk space 
usage. Each image has to be stored by the Docker daemon (which is efficient 
thanks to the layers mechanism), and its unpacked data need to be stored 
elsewhere, possibly duplicating data from base images.

We decided that the first option, directly mounting the layers of the image, is 
better due to its efficiency and despite the slightly larger implementation 
complexity and being tied to a specific Docker storage backend (support for 
alternative backends is feasible, but not planned for implementation).
