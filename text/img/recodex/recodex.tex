...
\tikzstyle{bordered} = [rounded corners=5pt,rectangle,draw,outer sep=0,inner sep=7,minimum size=10]
\tikzstyle{arrow} = [draw,-latex]
\tikzstyle{doublearrow} = [draw,latex-latex]

\node [bordered] (broker) {Broker};
\node [bordered, right=of broker] (api) {Business logic core\\(HTTP API)};
